	...
	QextSerialBase(name)
{
    ...
    std::cerr << "Win_QextSerialPort()" << std::endl;
    std::cerr << "PortSettings = " << settings.BaudRate << std::endl;
...
bool Win_QextSerialPort::open(OpenMode mode) {
    ...
    {
std::cerr << "QIODevice::NotOpen" << std::endl;
        return isOpen();
	}
...
std::cerr << "Win_QextSerialPort::open(OpenMode mode), isOpen() : " << isOpen() << std::endl;
...
std::cerr << "Win_QextSerialPort::open(OpenMode mode), BaudRate : " << Settings.BaudRate << std::endl;
            setBaudRate(Settings.BaudRate);
            setDataBits(Settings.DataBits);
            setStopBits(Settings.StopBits);
            setParity(Settings.Parity);
            setFlowControl(Settings.FlowControl);
            setTimeout(Settings.Timeout_Millisec);
std::cerr << "Settings Win_CommConfig baudRate with :" << Win_CommConfig.dcb.BaudRate << std::endl;
...
void Win_QextSerialPort::setFlowControl(FlowType flow) {
    ...
    if (!isOpen() && Win_Handle!=INVALID_HANDLE_VALUE) {
        switch(flow) {
...
void Win_QextSerialPort::setBaudRate(BaudRateType baudRate) {
    ...
    if (!isOpen() && Win_Handle!=INVALID_HANDLE_VALUE) {
        switch (baudRate) {
            ...
            /*500000 baud*/
            case BAUD500000:
                TTY_PORTABILITY_WARNING("Win_QextSerialPort Custom 500000 baud operation requested.");
                Win_CommConfig.dcb.BaudRate=500000;
                break;

            /*1000000 baud*/
            case BAUD1000000:
                TTY_PORTABILITY_WARNING("Win_QextSerialPort Custom 1000000 baud operation requested.");
