 ...
 : QIODevice(parent)
{
    ...
    construct();
    setQueryMode(EventDriven);
    platformSpecificInit();
}

/*!
Note that the name of the device used by a QextSerialPort constructed with this constructor will
be determined by #defined constants, or lack thereof - the default behavior is the same as
_TTY_LINUX_.  Possible naming conventions and their associated constants are:
...
See the other constructors if you need to open a different port.
*/
QextSerialPort::QextSerialPort(QueryMode mode, QObject *parent)
 : QIODevice(parent)
{
...
*/
QextSerialPort::QextSerialPort(const QString & name, QueryMode mode, QObject *parent)
    : QIODevice(parent)
{
    construct();
    setQueryMode(mode);
...
*/
QextSerialPort::QextSerialPort(const QextPortSettings& settings, QueryMode mode, QObject *parent)
    : QIODevice(parent)
{
    construct();
...
*/
QextSerialPort::QextSerialPort(const QString & name, const QextPortSettings& settings,
    QueryMode mode, QObject *parent)
    : QIODevice(parent)
{
    construct();
    setPortName(name);
...
{
    QByteArray data;
    qint64 totalAvail = bytesAvailable();
    if (totalAvail > 0) {
        qint64 readSoFar(0);
        qint64 bufferAvail = QIODevice::bytesAvailable();

        data.resize(totalAvail);
        if (bufferAvail)
            readSoFar = read(data.data(), qMin(static_cast<qint64>(data.size()), bufferAvail));
        if (totalAvail > readSoFar)
            readSoFar += readData(data.data() + readSoFar, data.size() - readSoFar);

        data.resize(readSoFar);
    }
    return data;
}

/*!
Sets the port settings of the serial port.
*/
void QextSerialPort::setPortSettings(const QextPortSettings &settings)
{
    ...
    setTimeout(settings.timeoutMsecs);
}

/*!
Returns the port settings of the serial port.
*/
QextPortSettings QextSerialPort::portSettings(void) const
{
    return _settings;
}
...
QString QextSerialPort::errorString()
{
    switch(lastErr)
    {
        ...
        case E_FILE_NOT_FOUND: return "The "+portName()+" file doesn't exists";
